...
class ReportGenerationService {
    ...
    async ensureReportsDirectory() {
        try {
            await fs_1.promises.access(this.reportsDir);
        }
        catch {
            await fs_1.promises.mkdir(this.reportsDir, { recursive: true });
        }
    }
    async generateCandidateReport(candidate, jobProfile, interviewAnalysis) {
    ...
    async generateCandidatePDF(candidate, jobProfile, interviewAnalysis) {
    ...
    async generateBatchSummaryReport(batch, candidates, jobProfile, interviewAnalyses) {
        const candidateReports = await Promise.all(candidates.map(candidate => {
            const interviewAnalysis = interviewAnalyses?.find(analysis => analysis.candidateId === candidate.id);
            return this.generateCandidateReport(candidate, jobProfile, interviewAnalysis);
        }));
        const completedCandidates = candidateReports.filter(report => report.completionStatus.scoringCompleted);
        ...
        const processingTime = batch.completedAt && batch.startedAt
            ? (batch.completedAt.getTime() - batch.startedAt.getTime()) / (1000 * 60 * 60)
            : 0;
        return {
            batch,
            jobProfile,
    ...
    async generateBatchSummaryPDF(batch, candidates, jobProfile, interviewAnalyses) {
    ...
    async exportCandidatesCSV(candidates, jobProfile, interviewAnalyses) {
    ...
    assessCompletionStatus(candidate) {
        return {
    ...
    generateCandidateHTML(report, interviewAnalysis) {
    ...
    generateBatchSummaryHTML(report) {
    ...
    generateScoreSection(candidate) {
    ...
    generateAIAnalysisSection(candidate) {
    ...
    generateLinkedInSection(candidate) {
    ...
    generateGitHubSection(candidate) {
    ...
    generateInterviewSection(candidate, interviewAnalysis) {
    ...
    generateRecommendationSection(candidate) {
        if (!candidate.finalScore) {
            return `
        <div class="section">
